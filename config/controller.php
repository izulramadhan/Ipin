<?php 
    ...
    class lsp{
        ...
        public function daftar($namalengkap,$notelp,$alamat,$username, $password){
            global $con;
            ...
            if($rows > 0){
                return ['response'=>'negative','alert'=>'User sudah terdaftar'];
            }else{
                $newpassword = base64_encode($password);

                $sql = "INSERT INTO table_user VALUES('','X','$namalengkap','$username','$newpassword','1537840377928.png','Admin','$notelp','$alamat')";
        ...
        public function insertskor($table, $values, $redirect,$nilaikesimpulan,$kesimpulan){
    		global $con;
    		$sql   = "INSERT INTO $table VALUES($values)";
            $query = mysqli_query($con,$sql);
            if($query){
                return ['response'=>'positive','alert'=>'TB/U = '.$nilaikesimpulan.' ('.$kesimpulan.')','redirect'=>$redirect];
            }else{
                echo mysqli_error($con);
                return ['response'=>'negative','alert'=>'Gagal Menambahkan Data'];
            }
        ...
        public function select2($table,$where,$whereValues){
            global $con;
            $sql = "SELECT * FROM $table WHERE $where = $whereValues";
            $query = mysqli_query($con,$sql);
            $data = [];
