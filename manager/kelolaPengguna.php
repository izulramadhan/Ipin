<?php 
    ...
    $data = $rg->select($table);
     ...
     if(isset($_POST['btnInput'])){
        ...
        $foto = $_FILES['foto'];
        $level = $_POST['level'];
        $redirect = "?page=kelPengguna";
        

        if($nama_user == "" || $username == "" || $password == "" || $confirm == "" || $level == ""){
            $response = ['response'=>'negative','alert'=>'Lengkapi Field !!!'];
        }else{
            $response = $rg->register($kd_user, $nama_user, $notelp, $alamat, $username, $password, $confirm, $foto, $level, $redirect);
        }
    }

    if(isset($_GET['delete'])){
        $response = $rg->delete($table,"kd_id",$_GET['id'],"?page=kelPengguna");
    }
                    ...
                    <div class="au-breadcrumb-content">
                       <div class="au-breadcrumb-left">
                            <ul class="list-unstyled list-inline au-breadcrumb__list">
                                ...
                                <li class="list-inline-item">Kelola Pengguna</li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-5">
                    <div class="card">
                        <div class="card-header">
                            <strong class="card-title mb-3">Input Pengguna</strong>
                        </div>
                        <div class="card-body">
                            <form method="post" enctype="multipart/form-data">
                                <div class="form-group">
                                    <!-- <label>Kode User</label> -->
                                    <input style="color: red; font-weight: bold;" class="au-input au-input--full" type="text" name="kd_user" readonly value="<?= $autokode; ?>" hidden>
                                </div>
                                <div class="form-group">
                                    <label>Nama Lengkap</label>
                                    <input class="au-input au-input--full" required type="text" name="nama_user" placeholder="Nama Lengkap">
                                </div>
                                <div class="form-group">
                                    <label>No. Telp</label>
                                    <input class="au-input au-input--full" required type="text" name="notelp" placeholder="Nomor Telp">
                                </div>
                                <div class="form-group">
                                    <label>Alamat</label>
                                ...
                                <button name="btnInput" class="au-btn au-btn--green m-b-20" type="submit">Input Pengguna</button>
                                       ...
                                       <script>
                                        $('#btnDelete<?php echo $no; ?>').click(function(e){
                                                      e.preventDefault();
                                                        ...
                                                        if (isConfirm) {
                                                            window.location.href="?page=kelPengguna&delete&id=<?php echo $dataB['kd_id'] ?>";
                                                        }
                                                      });
                                                    });
                                        </script>
